...
function seedDatabase() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield Chain_1.default.deleteMany({});
            yield Token_1.default.deleteMany({});
            // Seed chains
            console.log('🌐 Seeding chains...');
            const chains = yield Chain_1.default.insertMany(CHAINS_DATA);
            console.log(`✅ Created ${chains.length} chains: ${chains.map(c => c.name).join(', ')}`);
            // Generate token keys and seed tokens
            console.log('💎 Seeding tokens...');
            const tokensWithKeys = TOKENS_DATA.map(token => (Object.assign(Object.assign({}, token), { key: `${token.symbol.toLowerCase()}-${token.chainKey}` })));
            const tokens = yield Token_1.default.insertMany(tokensWithKeys);
